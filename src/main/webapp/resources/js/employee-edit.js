...
function setupDocument() {
    ...
    $(".trainerSelect").select2({
        ...
        ajax: {
            url: "/turfclubPrograms/trainersEmployees/service/trainers",
            dataType: 'json',
            type: "GET",
            ...
            results: function(data, page ) {
            	...
            	$.each( data, function( key, value ) {
            		...
            		dataReturn.push( {id:data[key][0] ,text: data[key][1] + " " +  data[key][2] } );
            		});
    ...
    initSelection: function(element, callback) {
    	...
    	callback({id: $(element).val(), text:  $(element).data("trainerName") });
	...
	$("#submitButton").click(
			function(e) {
				...
				if(earnings && (employmentStartYear == 2015)) {
	...
	// for both the current and previous year.
		...
		var yearEnding = d.getFullYear() - 1;
/*
	...
	$(".pension-button").click(
			function(e) {
				console.log($(this).data("dateTo"));
				console.log($(this).data("dateFrom"));
				updatePensionDetail($(this).data("dateFrom"),$(this).data("dateTo"),$(this).data("trainerId"),$(this).data("employeeId"));
			});
    
	
	//check pps for employee to ensure no dupliacte values
    checkPPsDetail($("#employee-id").val());
    setupEmploymentHistorySection();
...
function setupEmploymentHistorySection(){
...
function formatDateInput(dateInput){
	...
	if($(dateInput).val() != '' && $(dateInput).val() !== undefined && $(dateInput).val() != null){
		var today = new Date(Date.parse($(dateInput).val()));
...
function updatePensionDetail(startDate, endDate, trainerId, employeeId){
	
	
	$.ajax({
	    type: "GET",
	    url: "/turfclubPrograms/trainersEmployees/service/getHistories",
	    data: {trainerId : trainerId, employeeId: employeeId},
	    dataType: "json",
	    success: function(data){
	    	
	    console.log(JSON.stringify(data));
	   
	    if(data.multiplePPs == true){
	    	console.log("multiplePPS");
	    }
	    if(data.multiplePPs == false){
	    	console.log("multiplePPs None");
	    }
	    $('#earnings-table tr').remove();
	    var last = "";
	    var count = 0;
	    $.each(data.histories, function(index, itemData) {
	    	
	    	var sdate = new Date(itemData.ehDateFrom);
	    	var sYear = sdate.getFullYear();
	    	console.log(sYear + "  " + itemData.ehDateFrom);
	    	if(count == 0){
	...
	$("#pension-detail-modal").modal('show');
}


function checkPPsDetail(employeeId){
	
	
	$.ajax({
	    type: "GET",
	    url: "/turfclubPrograms/trainersEmployees/service/checkPPsNumbers",
	    data: {employeeId: employeeId},
	    dataType: "json",
	    success: function(data){
	    	
	    console.log(JSON.stringify(data));
	   console.log(data.validPPs);
	    if(data.validPPs == true){
	    	console.log("valid pps");
	    }
	    if(data.validPPs == false){
	    	console.log("two many pps");
	    	$.each(data.multiplePpsList, function(index, itemData) {
