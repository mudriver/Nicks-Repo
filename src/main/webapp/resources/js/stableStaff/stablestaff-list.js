...
function setupDocument() {
 ...
 $('#listEmployeeForm')
	...
	$('#leftHeader').text($('#leftHeader').text().replace("year" , year-1));
}
...
function download() {
	...
	$.get('/turfclubPrograms/trainersEmployeesOnline/download/token', function(response) {
		// Store token
		...
		// Start download
		$("#pdf-section").show();
		console.log($("#trainerId").val());
		$('#pdf-preview').html('<object id="pdf" standby="please wait for pdf to load" type="application/pdf" data="/turfclubPrograms/trainersEmployeesOnline/employeeReportDownload/' + $("#trainerId").val() +  '?'+'token='+token +'" width="800" height="1000"></object>');


		// Check periodically if download has started
		var frequency = 1000;
		var timer = setInterval(function() {
			$.get('/turfclubPrograms/trainersEmployeesOnline/download/progress', {token: token}, 
					function(response) {
