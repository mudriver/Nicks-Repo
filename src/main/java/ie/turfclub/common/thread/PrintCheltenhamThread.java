...
public class PrintCheltenhamThread implements Runnable {

	private TrainersService trainerService;
	private String cheltenhamPDFPath;
	private HttpSession session;
	
	public PrintCheltenhamThread(TrainersService trainerService, String cheltenhamPDFPath, HttpSession session) {
		this.trainerService = trainerService;
		this.cheltenhamPDFPath = cheltenhamPDFPath;
	...
	public void run() {
		...
		File file = new File(this.cheltenhamPDFPath);
		if(file.exists())
			file.delete();
		...
		try {
			PdfWriter.getInstance(document, new FileOutputStream(this.cheltenhamPDFPath));
			document.open();
	...
	private PdfPTable addContentIntoPDF(List<HashMap<String, Object>> records) {
		...
		cell = new PdfPCell(new Phrase(new Chunk("Cheltenham", titleFont)));
		cell.setBorder(0);
		table.addCell(cell);
		...
		session.setAttribute("cheltenhamStatus", "done");
		return table;
	}
}
