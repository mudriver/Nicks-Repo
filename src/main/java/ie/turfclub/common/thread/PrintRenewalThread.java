...
public class PrintRenewalThread implements Runnable {

	private TrainersService trainerService;
	private String aintreePDFPath;
	private HttpSession session;
	
	public PrintRenewalThread(TrainersService trainerService, String aintreePDFPath, HttpSession session) {
	...
	private PdfPTable addContentIntoPDF(List<HashMap<String, Object>> records) {
		...
		cell = new PdfPCell(new Phrase(new Chunk("All Trainers Report - Accounts", titleFont)));
		cell.setColspan(4);
		cell.setBorder(0);
		table.addCell(cell);
		...
		cell.setBorderColor(BaseColor.BLUE);
		table.addCell(cell);
		cell = new PdfPCell(new Phrase(new Chunk("Card Type", headerFont)));
		cell.setBorder(PdfPCell.TOP | PdfPCell.BOTTOM);
		cell.setBorderColor(BaseColor.BLUE);
		table.addCell(cell);
		cell = new PdfPCell(new Phrase(new Chunk("Card Number", headerFont)));
		cell.setBorder(PdfPCell.TOP | PdfPCell.BOTTOM);
		cell.setBorderColor(BaseColor.BLUE);
		table.addCell(cell);
		...
		if(records != null && records.size() > 0) {
			for (HashMap<String, Object> map : records) {
				if((boolean)map.get("isTrainer")) {
					...
					cell.setBorder(0);
					table.addCell(cell);
					cell = new PdfPCell(new Phrase(new Chunk((String)map.get("surname")+" "+(String)map.get("firstname"), employeeTextFont)));
					cell.setBorder(0);
					table.addCell(cell);
					String cardType = ((String)map.get("cardType") != null) ? (String)map.get("cardType") : "";
					cell = new PdfPCell(new Phrase(new Chunk(cardType, employeeTextFont)));
					cell.setBorder(0);
					table.addCell(cell);
					String cardNumber = ((Integer)map.get("cardNumber") != null && (Integer)map.get("cardNumber") > 0) ? String.valueOf(map.get("cardNumber")) : "";
					cell = new PdfPCell(new Phrase(new Chunk(cardNumber, employeeTextFont)));
					cell.setBorder(0);
					table.addCell(cell);
				}
			}
		...
		session.setAttribute("printRenewalStatus", "done");
		return table;
	}
}
